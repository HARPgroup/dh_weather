...
raster2pgsql "$fil.conus-4326.tif" tmp_mtd_precipgrid > tmp_mtd_precipgrid.sql

# add to database
echo "drop table tmp_mtd_precipgrid;" | psql -h dbase2 drupal.beta 
cat tmp_mtd_precipgrid.sql | psql -h dbase2 drupal.beta 
echo "alter table tmp_mtd_precipgrid add column dataset varchar(32);" | psql -h dbase2 drupal.beta 
clip="insert into tmp_mtd_precipgrid (rast, dataset) "
clip="$clip select st_clip(a.rast, st_envelope(st_setsrid(b.dh_geofield_geom,4326))), 'va_precip_points' "
#clip="$clip from tmp_mtd_precipgrid as a left outer join field_data_dh_geofield as b "
clip="$clip from tmp_mtd_precipgrid as a "
...
select rid, st_astext(st_envelope(rast)) from tmp_mtd_precipgrid group by rid;
...
left outer join tmp_mtd_precipgrid as c 
on (
